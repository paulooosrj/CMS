...
function geraBKPeAplica(){
		...
		if($_POST['bkp']=='true'){
			...
			$file_exists_dir->set_insert('responsavel',$user->get('id'));
...
function getVersionsFile(){
	if(empty($user->get('id')) || $user->get('id')==""){echo "window.location.reload()";exit;}
	$pathFile 	= $_REQUEST['pathFile'];
	$pathFile 	= explode("/",$pathFile);
	$file 		= end($pathFile);
	$pathFile 	= array_slice($pathFile,0, -1);
	...
	if($count==0){
		...
		foreach($file_exists_dir->fetch_array as $file){
			if($file['checkin']=='1'){
					...
					if($file['id_checkout']==$user->get('id')){
		...
		if($arrayMysql['checkin']=='1' && $arrayMysql['id_checkout']==$user->get('id')){
...
function loadFileVersion(){
	if($user->get('id')==""){echo "window.location.reload()";exit;}
...
function saveFileBKP(){
			...
			$U->set_update('responsavel_altera',$user->get('id'));
			if($U->salvar()){
